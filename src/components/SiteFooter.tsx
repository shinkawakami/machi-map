 ...
 * 3つのデータ源の権利者（国土地理院・国土交通省）は畳んだ状態でも名乗っていて、
 * 加工した旨も出ている。地図まわりで広く使われている畳み方と同じ姿勢に揃える。
 *
 * **地図右下の attribution を当てにしない。**
 * `AttributionControl({ compact: true })` は広い画面では常設で出ているが、
 * **狭い画面では下のシートが地図の右下を覆うので、ⓘ ごと見えない**
 * （シートは `absolute inset-x-0 bottom-0` で、畳まないかぎり必ず重なる）。
 * 「畳んである」のではなく不在なので、背景地図の出典をあちらに任せる形にはできない。
 * つまり**狭い画面では、この畳んだ1行が唯一の出典表示**になる。だから削らない。
 */
