...
export default function ShelterFilterBar({
  ...
  */
  ...
  return (
    <div className="flex shrink-0 items-center gap-1.5 border-b border-zinc-100 px-3 py-1.5">
      {KINDS.map((kind) => {
      ...
      */}
      ...
      >
        <summary
          className={`cursor-pointer list-none rounded-full border px-2 py-0.5 text-[11px] marker:content-none ${
            selected
...
function DisasterChip({
  ...
  return (
    <button
      ...
      title={title}
      onClick={onClick}
      className={`shrink-0 rounded-full border px-2 py-0.5 text-[11px] whitespace-nowrap transition-colors ${
        selected
          ? "border-zinc-900 bg-zinc-900 font-medium text-white"
