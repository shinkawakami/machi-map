      ...
      <div className="pointer-events-auto w-full max-w-xs rounded-xl border border-zinc-200 bg-white p-4 shadow-xl">
          ...
          {PLACE_PRESETS.map((preset) => (
            ...
            >
              ...
              {usedNames.includes(preset) && (
                <span className="ml-1 text-[10px] text-zinc-400">上書き</span>
              )}
            </button>
          ))}
        </div>
        ...
        <div className="mt-3 flex items-center gap-1.5">
          <input
            ...
            placeholder="ほかの名前"
            maxLength={12}
            className="min-w-0 flex-1 rounded-lg border border-zinc-200 px-2 py-1.5 text-sm text-zinc-900 placeholder:text-zinc-400"
          />
          <button
            type="button"
            disabled={!cleaned}
